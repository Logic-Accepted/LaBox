...
# 2024.9.27更新
最近半月似乎提高了检测频率，每日凌晨的三点依然有一次检测封号，而另一次则不定，最近一次观察到在晚上的 12：00 左右。我借此机会再次升级了网络设备，购入一台升腾 j1900 工控机，安装了 pve 并安装 openwrt 作为其网关，现在充当软路由替换掉 K2 路由器并把 Ax6s 作为 AP 使用，经过这样的升级之后可以看得出网络设备总体的性能显著提升了，宿舍内各位可以在十多台设备同时连接的时候放心下载，基本不会让其他设备的延迟波动，或许这就是 WIFI6 的优势吧。目前修改 User-Agent 选用了 UA3F 的方案，并尝试针对 DPI 检测把具有 QQ 和微信特征的流量进行了 TLS 加密，但遗憾的是仍然会被检测封号，只是被封号恢复到了最多每日一次的频率。目前我不能确定两者是否存在关联，但先这样用着吧。上次更新时提到的流量全量加密方案仍在有一搭没一搭的摸索，或许下次被封禁的时候就用上了。   
顺便一讲，Bilibili up主 yewenxuan64 关于校园网防检测的[视频](https://www.bilibili.com/video/BV1yr4meeENt/?spm_id_from=333.337.search-card.all.click)终于上线了，视频相当长，基本列举了全部的检测措施和她所做出的尝试经历。虽然不论是语气、叙述方式还是个别分镜头都和预告片一样充满一股用力过猛的感觉，但作为自己的折腾经验分享，它有非常清晰的逻辑和非常炫酷的动效，我觉得确实是相当不错的，至少比我这篇字儿要强得多。（财力也要比我雄厚的多啊啊啊）  
